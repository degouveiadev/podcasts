...
const Podcast: NextPage<PodcastProps> = ({ clip, statusCode }: PodcastProps) => {
  const { Link } = Routes;
...
Podcast.getInitialProps = async ({ query }: NextPageContext) => {
  try {
    const id = query.id;
    const fetchClip = await fetch(`${Config.URL}audio_clips/${id}.mp3`);
